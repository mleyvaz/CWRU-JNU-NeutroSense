...
heading(doc, "4.6. Seven Extensions Toward a Demonstrated Neutrosophic Advantage", level=2)
body(doc, (
    ...
    "advantage over predictive entropy alone. Rather than stop there, we ran seven further, genuinely "
    ...
    "argument holds when actually measured; (iii)-(iiib) whether I2-hat's weak signal is an artifact of "
    "using only three, coarsely-disagreeing base learners, and whether any resulting significance "
    "survives validation; (iv) whether I1-hat and I2-hat respond differently to two mechanistically "
    "different kinds of perturbation; (v) whether re-deriving I1-hat and I2-hat directly from the "
    "algebraic decomposition already presented in Section 3.5, rather than using entropy-of-the-average "
    "and a discrete vote count, changes any of the above; and (vi) whether making T-hat and F-hat "
    "genuinely independent (not two entries of one softmax vector) and applying established "
    "neutrosophic score functions from the decision-making literature reveals value the conventional "
    "operationalization misses. None of these were run before the manuscript's "
...
body(doc, (
    ...
    "The partial-correlation significance is a genuine, non-oracle statistical result -- the first in "
    "this study showing I2-hat carries real independent information -- but the 0.001490 AURC "
    "improvement is still an ORACLE quantity (the joint rule's threshold pair is selected by searching "
    "the test set itself), and Section 4.6.3b below tests directly whether it survives honest "
    "validation (larger_ensemble_i2_jnu.py)."
))
body(doc, (
    "4.6.3b. Does the 8-model signal survive validation? We repeated Section 4.6.1's three-way "
    ...
    "test in Section 4.6.1. The statistically significant partial correlation reported in Section "
    "4.6.3 does not translate into a validated selective-classification advantage: the positive AURC "
...
body(doc, (
    "4.6.5. Redefining I1-hat and I2-hat from the decomposition itself. Section 3.5 already derives, "
    "but never uses as the actual working indicators, the exact algebraic identity H(P_avg) = "
    "mean_m[H(P_m)] + mean_m[KL(P_m||P_avg)]. We re-ran the full statistical battery using its two "
    "additive terms directly: I1-new = mean_m[H(P_m)] (each base learner's own entropy, averaged -- "
    "aleatoric-like) and I2-new = mean_m[KL(P_m||P_avg)] (each learner's divergence from the ensemble "
    "average, averaged -- epistemic-like, continuous rather than a coarse discrete vote count). The "
    "identity holds exactly on this data (max floating-point gap 3.16e-08), a sanity check the "
    "original (total entropy, vote-disagreement) pair cannot even pose, since vote disagreement is not "
    "an additive component of H(P_avg). The new pair is also markedly less correlated with each other "
    "(r=0.318) than the original pair (r=0.746), addressing the redundancy concern raised elsewhere in "
    "this section. On the SAME main-pipeline ensemble and test split as Table 1 (fit on 600+800 rpm, "
    "test on 1000 rpm), I2-new's partial correlation with error controlling for T-hat and F-hat is "
    "r=+0.2416 (p=1.43e-78) -- far larger and more significant than the original I2-hat's r=+0.017 "
    "(p=0.20, n.s.); I1-new's is r=-0.0692 (p=1.15e-07), small, significant, and negatively signed, "
    "consistent with most of the error-relevant signal having moved into the epistemic term. As "
    "standard (non-oracle) selective-classification metrics requiring no threshold search -- the same "
    "kind of number as every AURC in Table 3b -- I2-new alone reaches AURC=0.3859, better than the "
    "original I1-hat's 0.3915, and a fixed, equal-weight linear combination I1-new+I2-new reaches "
    "AURC=0.3680, a genuine, non-oracle improvement of 0.0235 (about 6% relative) over the original "
    "I1-hat. The hidden-risk-zone gap also widens, from 28.2% vs. 63.7% error to 23.3% vs. 71.8%. Only "
    "the further step of searching a joint (tau1, tau2) threshold PAIR is an oracle quantity here "
    "(AURC=0.3773, a further improvement over the fixed combination) -- and, following the pattern "
    "established in 4.6.1 and 4.6.3b, we tested whether it survives honest validation. Using the same "
    "fit-600/validate-800/test-1000 protocol, I2-new alone (threshold selected on validation) reaches "
    "58.61% test risk at 34.02% test coverage, sharply better than I1-new alone's 78.66% risk at 79.76% "
    "coverage (the two are not at matched coverage, so this specific comparison should be read "
    "qualitatively); the searched joint rule reaches 58.63% risk at 34.12% coverage -- statistically "
    "indistinguishable from I2-new alone, meaning the oracle joint-threshold search adds nothing beyond "
    "what I2-new alone already achieves once properly validated (validated_decomposed_indeterminacy_"
    "jnu.py). The demonstrated, non-oracle advantage of this section is therefore specifically "
    "attributable to REDEFINING I2-hat using the decomposition's own epistemic term, not to any joint "
    "thresholding rule over the two axes."
))
body(doc, (
    ...
    "original or new with I1-new from Section 4.6.5. Every configuration performed WORSE than entropy "
    "alone (AURC 0.4062-0.4348, versus I1-hat alone's 0.3915) and far worse than Section 4.6.5's fixed "
    ...
    "formulas improved on what a conventional entropy decomposition already achieves "
    "(independent_tif_neutrosophic_score_jnu.py)."
))
body(doc, (
    "Taken together, five of these seven results (4.6.1, 4.6.2, 4.6.3b, 4.6.4, 4.6.6) reinforce the paper's "
    "central negative finding through different mechanisms: a properly validated joint rule does not "
    "beat entropy alone, whether with 3 or 8 base learners; a measured cost asymmetry runs opposite to "
    "what any practical triage argument needs; a real distribution shift decreases rather than "
    "raises either indicator; and neither genuinely independent T-hat/F-hat nor established "
    "neutrosophic score functions from the literature improve on conventional entropy. Two results are "
    "genuinely positive, and importantly, of different "
    "evidentiary strength. 4.6.3's statistically significant partial correlation (8-model ensemble) "
    "does NOT survive validation (4.6.3b) -- it remains oracle-only. 4.6.5, by contrast, IS a "
    "non-oracle, validated-consistent improvement: redefining I2-hat as the decomposition's own "
    "epistemic term (mean KL-divergence of each base learner from the ensemble average, in place of a "
    "coarse discrete vote count) yields a standard, no-threshold-search AURC of 0.3859, and a fixed "
    "equal-weight combination with the correspondingly redefined I1-hat reaches AURC=0.3680 -- both "
    "genuinely better than the original I1-hat's 0.3915, without searching any threshold on the test "
    "set. Only the further step of searching a joint threshold pair remains an oracle quantity, and it "
    "adds nothing once validated. The demonstrated advantage in this study is therefore specific and "
    "narrow: it comes from operationalizing I2-hat correctly (as the decomposition already implied but "
    "the original pipeline did not use), not from any joint decision rule over two separately-scored "
    "axes. None of Section 4.6's results has been through the adversarial review process the rest of "
    "this manuscript underwent; we flag them explicitly as newer, less scrutinized material for that "
    "reason."
))
...
body(doc, (
    ...
    "than restating it. Five reinforce the negative finding through different mechanisms: a properly "
    "validated (not oracle) joint rule does not generalize better than entropy alone on truly held-out "
    "data, whether with 3 or 8 base learners (the latter's statistically significant partial "
    "correlation, p=4.85e-08, does not survive validation either); a measured, rather than assumed, "
    ...
    "neutrosophic score functions from the decision-making literature improve on conventional entropy. "
    "One test is genuinely, non-oracle positive: redefining I1-hat and I2-hat as the two additive terms "
    "of the entropy decomposition already derived in Section 3.5 (mean per-model entropy and mean "
    "KL-divergence from the ensemble average, respectively) yields a standard AURC of 0.3680 for a "
    "fixed-weight combination, genuinely better than the original I1-hat's 0.3915, without searching "
    "any threshold on the test set -- though a further joint-threshold-search rule over these redefined "
    "indicators still does not survive validation beyond what the redefined I2-hat already achieves "
    "alone. None of Section 4.6's "
