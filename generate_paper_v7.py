...
body(doc, (
    "Taken together, all four extensions ultimately reinforce the paper's central negative finding, "
    "though through different mechanisms and to different degrees. Three (4.6.1, 4.6.2, 4.6.4) do so "
    "directly: a properly validated rule does not beat entropy alone, a measured cost asymmetry runs "
    "opposite to what any practical argument needs, and a real distribution shift decreases rather than "
    "raises either indicator. The fourth (4.6.3) initially looked like a genuine positive result -- "
    "ensemble diversity makes I2-hat's independent contribution statistically significant for the first "
    "time in this study -- but 4.6.3b shows that significance does not survive the same honest "
    "validation protocol used in 4.6.1: with 8 base learners, the validated joint rule still does not "
    "beat entropy alone (74.85% vs. 74.81% test risk), by almost exactly the same negligible margin as "
    "with 3 learners. Ensemble diversity makes I2-hat statistically real without making it practically "
    "usable by any rule we have tested honestly. None of these five results (4.6.1-4.6.4, 4.6.3b) has "
    "been through the adversarial review process the rest of this manuscript underwent; we flag them "
    "explicitly as newer, less scrutinized material for that reason."
))
