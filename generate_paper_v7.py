...
# Section 4.6 (the seven exploratory extensions) has NOT been through this
# manuscript's own adversarial review process, unlike Sections 1-6, and its
# self-referential commentary about that fact is appropriate for an internal/
# repo record but not for a journal submission. Building with
# INCLUDE_SECTION_46=False (the "--submission" CLI flag) produces the
# submission-ready manuscript without it; the full version (with Section 4.6)
# remains available via the default build for internal review and the
# public repo.
INCLUDE_SECTION_46 = "--submission" not in sys.argv
...
_abstract_446_sentence = (
    ...
    "non-oracle improvement (AURC 0.3682 for a fixed-weight combination vs. 0.3915 for the original "
    "I1-hat), though a further joint-threshold-search rule over these redefined indicators does not "
    "survive honest validation beyond what the redefined I2-hat already achieves alone. "
) if INCLUDE_SECTION_46 else ""
body(doc, (
...
body(doc, (
    ...
    + (
        " Sixth, we identify and validate a genuine, "
        ...
        "from 0.3915 to 0.3682, confirmed under honest fit/validate/test discipline, not merely a "
...
if INCLUDE_SECTION_46:
    ...
    body(doc, (
        ...
        "and a discrete vote count, raises selective-classification AURC from 0.3915 to 0.3682 with no "
    ...
    body(doc, (
        "4.6.1. The positive result: redefining I1-hat and I2-hat from the decomposition itself, corrected "
        "after independent multi-model adversarial review. Section 3.5 already derives, but never uses as "
        "the actual working indicators, the exact algebraic identity H(P_avg) = mean_m[H(P_m)] + "
        "mean_m[KL(P_m||P_avg)]. We re-ran the full statistical battery using its two additive terms "
        "directly: I1-new = mean_m[H(P_m)] (each base learner's own entropy, averaged -- aleatoric-like) "
        "and I2-new = mean_m[KL(P_m||P_avg)] (each learner's divergence from the ensemble average, "
        "averaged -- epistemic-like, continuous rather than a coarse discrete vote count). The identity "
        "holds exactly on this data (max floating-point gap 3.16e-08); a direct consequence, caught by an "
        "independent adversarial review after this section's first draft, is that an UNSTANDARDIZED sum "
        "I1-new+I2-new is algebraically identical to H(P_avg) and so MUST reproduce the original I1-hat's "
        "own AURC exactly (verified: 0.391543, matching to six decimals) -- a raw sum has no freedom to "
        "improve on entropy alone by construction. Any genuine gain can only come from a WEIGHTING or "
        "standardization step, and that review also caught that our first version of this step standardized "
        "using the TEST set's own mean and standard deviation -- a deployment-invalid, test-set-dependent "
        "computation, despite this section's text claiming \"no threshold search on the test set.\" We "
        "corrected this by computing I1-new's and I2-new's mean and standard deviation on the TRAINING set "
        "only (600+800 rpm, the same data the ensemble itself was fit on) and using those fixed constants "
        "to standardize the test-set values before summing. The corrected, deployment-honest combination "
        "reaches AURC=0.3682 -- nearly identical to the original (leaky) 0.3680, confirming the underlying "
        "gain was real, not a leakage artifact, but was mis-described. I2-new alone reaches AURC=0.3859, "
        "better than the original I1-hat's 0.3915, though I2-new's own improvement over the original "
        "(0.0056) accounts for only about a quarter of the corrected combination's total improvement "
        "(0.0233); most of the gain comes from the differential weighting the standardization applies, not "
        "from I2-new in isolation -- we do not attribute \"the entire gain\" to I2-new, a claim an earlier "
        "draft of this section made and that a reviewer correctly rejected. We further validated the "
        "corrected combination end-to-end under the fit-600/validate-800/test-1000 protocol used "
        "throughout this section, computing its full AURC (not a single coverage point) with "
        "standardization constants fixed on the 600 rpm fit set alone: AURC=0.5848 on the true test set, "
        "versus 0.7294 (I1-new alone) and 0.5964 (I2-new alone) under the same weaker, single-condition "
        "ensemble -- the combination's advantage survives a fully independent re-fit, not only a "
        "different standardization source within the main pipeline's own ensemble "
        "(validated_decomposed_indeterminacy_jnu.py). The new (I1-new, I2-new) pair is also markedly less "
        "correlated with each other (r=0.318) than the original pair (r=0.746), and the hidden-risk-zone "
        "gap widens from 28.2% vs. 63.7% error to 23.3% vs. 71.8%. Two further corrections, both from the "
        "same review: an oracle search over a joint (tau1, tau2) threshold pair reaches AURC=0.3773, which "
        "is WORSE than the corrected fixed combination (0.3682), not \"a further improvement\" as an "
        "earlier draft stated (lower AURC is better; we had the comparison direction backwards); and this "
        "joint search uses a 200-quantile grid for I2-new's continuous values (it has 5,634 unique values "
        "in this data), so it is an approximate grid oracle, not a mathematically exact one, and we no "
        "longer call it \"exact.\" Section 4.6.6 shows that going further still -- making T-hat and F-hat "
        "genuinely independent, or applying the neutrosophic literature's own score functions -- does not "
        "improve on this result either. The demonstrated advantage in this section is real, corrected, and "
        "validated twice over, but its source is specific: a properly standardized combination of two "
        "conventional entropy-decomposition terms, not I2-hat alone and not any oracle joint rule "
        "(decomposed_indeterminacy_jnu.py)."
    ))
    body(doc, (
    ...
    body(doc, (
        ...
        "average), giving I2-hat up to nine possible discrete levels instead of four (eight are actually "
        "observed on this test set; the ninth, full 8-of-8 disagreement, does not occur). Individual model accuracies on JNU's "
    ...
    body(doc, (
        "4.6.6. Attempted independent T-hat/F-hat, and established neutrosophic score functions -- with a "
        "logical flaw in the first design, also caught by independent review. Every T-hat/F-hat pair used "
        "elsewhere in this paper is two order statistics of the SAME softmax output (T-hat=P1, F-hat=P2), "
        "so T-hat+F-hat<=1 always -- conventional probability, not the neutrosophic property that T, I and "
        "F need not sum to 1 and can in principle be assessed independently. We attempted this by training "
        "two SEPARATE logistic-regression meta-classifiers, via 5-fold out-of-fold stacking on the training "
        "conditions only, estimating T-new = P(the ensemble's predicted class is correct) and F-new = "
        "P(the runner-up class is actually correct). T-new+F-new exceeds 1 for 23.84% of test instances, "
        "confirming the algebraic sum constraint is genuinely absent -- but two independent reviewers "
        "correctly identified a deeper problem with this design: the two meta-labels are MUTUALLY "
        "EXCLUSIVE BY CONSTRUCTION (the true class cannot simultaneously be both the top-1 predicted class "
        "and the distinct runner-up class), so a strong negative correlation between the fitted T-new and "
        "F-new is close to guaranteed regardless of whether the underlying evidence is independent -- the "
        "observed r=-0.9998 is very likely an artifact of this label design, not a measurement of genuine "
        "evidential (in)dependence, and we cannot treat this attempt as informative about neutrosophic "
        "independence either way. A design where T and F are estimated from evidence sources that do not "
        "share this structural exclusivity (for example, physically distinct sensor channels or feature "
        "domains) would be needed to test this properly; we did not attempt that here and flag it as the "
        "correct next step rather than a result. Separately, we applied established neutrosophic score and "
        "accuracy functions from the decision-making literature -- S(T,I,F)=(2+T-I-F)/3 and A(T,F)=T-F -- "
        "as selective-classification scores, using every combination of (T-hat, F-hat) original or new "
        "with I1-new from Section 4.6.1. Every configuration performed WORSE than entropy alone (AURC "
        "0.4062-0.4348, versus I1-hat alone's 0.3915) and far worse than Section 4.6.1's corrected fixed "
        "combination (AURC=0.3682); A(T-hat,F-hat)=T-hat-F-hat exactly reproduces the margin baseline "
        "already reported in Table 3b (AURC=0.4220), as expected since it is algebraically the same "
        "quantity. This part of the test is not undermined by the labeling flaw above (it uses the "
        "original T-hat/F-hat as well as the flawed T-new/F-new, and both perform worse either way), so "
        "the finding that established neutrosophic aggregation formulas do not improve on Section 4.6.1's "
        "conventional entropy decomposition stands (independent_tif_neutrosophic_score_jnu.py)."
    ))
    body(doc, (
        ...
        "vote count -- yields a standard, no-threshold-search AURC of 0.3859 alone, and 0.3682 for a fixed "
        "equal-weight combination with the correspondingly redefined I1-hat, both genuinely better than the "
        "original I1-hat's 0.3915 (Section 4.6.1). An earlier version of this specific result was corrected "
        "after independent multi-model adversarial review found that its combination step had been "
        "standardized using the TEST set's own statistics; recomputed using only training-set statistics, "
        "the corrected AURC (0.3682) is nearly unchanged, and a further, fully independent re-fit "
        "(fit-600/validate-800/test-1000) confirms the combination still beats both single indicators, so "
        "the underlying gain appears real rather than a leakage artifact -- but this correction is itself "
        "the reason we describe Section 4.6 as unreviewed, less scrutinized material relative to Sections "
        "1-6. The remaining six results define this finding's "
...
if INCLUDE_SECTION_46:
    body(doc, (
        ...
        "standard AURC of 0.3682 for a fixed-weight combination, genuinely better than the original "
...
out_name = "CWRU_JNU_NeutroSense_v7.docx" if INCLUDE_SECTION_46 else "CWRU_JNU_NeutroSense_v7_SUBMISSION.docx"
