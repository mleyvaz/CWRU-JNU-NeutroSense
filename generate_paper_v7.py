...
heading(doc, "4.6. Six Extensions Toward a Demonstrated Neutrosophic Advantage", level=2)
body(doc, (
    ...
    "advantage over predictive entropy alone. Rather than stop there, we ran six further, genuinely "
    ...
    "different kinds of perturbation; and (v) whether re-deriving I1-hat and I2-hat directly from the "
    "algebraic decomposition already presented in Section 3.5, rather than using entropy-of-the-average "
    "and a discrete vote count, changes any of the above. None of these were run before the manuscript's "
...
body(doc, (
    "Taken together, four of these six results (4.6.1, 4.6.2, 4.6.3b, 4.6.4) reinforce the paper's "
    ...
    "what any practical triage argument needs; and a real distribution shift decreases rather than "
    "raises either indicator. Two results are genuinely positive, and importantly, of different "
...
body(doc, (
    "Section 4.6 reports five further, genuinely new tests aimed directly at that open question rather "
    "than restating it, and all five ultimately reinforce the negative finding. A properly validated "
    "(not oracle) joint rule does not generalize better than entropy alone on truly held-out data; a "
    "measured, rather than assumed, audit-reliability asymmetry runs in the opposite direction from "
    "what any practical triage argument requires; out-of-distribution inputs decrease, rather than "
    "increase, both indicators, revealing a confident-extrapolation failure mode neither indicator "
    "flags; and while eight base learners instead of three make I2-hat's partial contribution "
    "statistically significant for the first time in this study (p=4.85e-08), repeating the same "
    "validated protocol with that larger ensemble shows this significance does not translate into a "
    "validated selective-classification advantage either (74.85% vs. 74.81% test risk, essentially the "
    "same negligible gap as with three learners). Ensemble diversity makes I2-hat statistically real "
    "without making it practically usable by any rule tested so far. None of Section 4.6's "
